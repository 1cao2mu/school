...
public class Tab1FindDetailActivity extends BaseActivity {
    ...
    private void initData() {
        try {
            if (adapter != null) {
                if (page == 1) {
                    adapter.refresh(Data.getCommitData(helper, page, object.getInt("fid")));
                } else {
                    adapter.append(Data.getCommitData(helper, page, object.getInt("fid")));
                }
            } else {
                adapter = new PingLunListAdapter(Data.getCommitData(helper, page, object.getInt("fid")), getContext());
                adapter.setOnItemChildViewClickListener(Tab1FindDetailActivity.this);
    ...
    private void add() throws JSONException {
        ...
        long i = db1.insert("findTabCommit", null, values);
        if (i == -1) {
            //失败
            ...
            int num = db1.update("findTab", values, "fid = ?", new String[]{String.valueOf(object.getInt("fid"))});
            db1.close();
            if (num < 1) {
                Utils.showToast(getContext(), "操作失败");
            } else {
